...
def main():
    ...
    if mode == "config":
        ...
        project_name = st.text_input("Project Name", config.project_name)
        ...
        with col2:
            ...
            with limit_col1:
                snowball_limit = st.number_input("Snowball Limit", value=config.search_criteria.snowball_from_keywords_limit)
            with limit_col2:
                min_citations = st.number_input("Min Citations", value=config.search_criteria.min_citations)
        ...
        with st.expander("⚙️ Advanced Settings", expanded=False):
            ...
            with adv1:
                ...
                model_screening = st.text_input("Screening Model", config.llm_settings.model_screening)
                model_extraction = st.text_input("Extraction Model", config.llm_settings.model_extraction)
            with adv2:
        ...
        st.info(f"Current project: **{config.project_name}**")
